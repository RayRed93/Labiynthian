...
public class DungeonArmSwinger : MonoBehaviour {
	...
	public string GithubProjectAndDocs = "https://github.com/ElectricNightOwl/DungeonArmSwinger";


    [SerializeField]
    private GameObject VRCamera;
    [SerializeField]
    private GameObject VRRoot;
	...
	[Tooltip("General - Auto Adjust Fixed Timestep\n\nIn order for DungeonArmSwinger to handle movement and wall collisions correctly, Time.fixedDeltaTime must be 0.0111 (90 per second) or less.  If this feature is enabled, the setting will be adjusted automatically if it is higher than 0.0111.  If disabled, an error will be generated but the value will not be changed.\n\n(Default: true)")]
	...
	[SerializeField]
	[Tooltip("Arm Swing - Button\nOnly if Arm Swing Navigation is enabled\n\nDefines which controller button is used to activate DungeonArmSwinger.  The button is the same on both controllers.\n\n(Default: Grip button)")]
	...
	[Tooltip("Raycast - Ground Layer Mask\n\nLayers that DungeonArmSwinger will consider 'the ground' when determining Y movement of the play space and when calculating angle-based prevention methods. \n\n(Default: Everything)")]
	...
	[Tooltip("Prevent Wall Clip - Layer Mask\nOnly if Prevent Wall Clip is enabled\n\nLayers that DungeonArmSwinger will consider 'walls' when determining if the headset has gone out of bounds.\n\n(Default: Everything)")]
	...
	void Awake() {
        ...
        headsetGameObject = VRCamera;// GameObject.FindObjectOfType<SteamVR_Camera>().gameObject;
        cameraRigGameObject = VRRoot;// GameObject.FindObjectOfType<SteamVR_ControllerManager>().gameObject;
	...
	void FixedUpdate() {
		...
		// Doing this in Update() allows the Camera Rig to be scaled during runtime but keep the same DungeonArmSwinger feel
    ...
    void verifySettings() {
        ...
        if (!this.GetComponent<SteamVR_ControllerManager>()) {
            Debug.LogError("DungeonArmSwinger.verifySettings():: DungeonArmSwinger is applied on a GameObject that is not a SteamVR CameraRig, or is a CameraRig without a SteamVR Controller Manager.  Please review the DungeonArmSwinger instructions.  DungeonArmSwinger will fail.");
        }

        // Rewind Settings
        if (rewindNumSavedPositionsToRewind > rewindNumSavedPositionsToStore) {
            Debug.LogError("DungeonArmSwinger.verifySettings():: rewindNumSavedPositionsToRewind is greater than rewindNumSavedPositionsToStore, rewinding will fail.");
        }
		...
		if (Time.fixedDeltaTime > 1f/90f) {
			if (generalAutoAdjustFixedTimestep) {
				Debug.LogWarning("DungeonArmSwinger.verifySettings():: Fixed Timestep is set to " + Time.fixedDeltaTime + ".  Since you have generalAutoAdjustFixedTimestep set to true, DungeonArmSwinger will auto adjust this value to " + 1f / 90f + " (90 steps per second) for you.");
				Time.fixedDeltaTime = 1f / 90f;
			} else {
				Debug.LogError("DungeonArmSwinger.verifySettings():: Fixed Timestep is set to " + Time.fixedDeltaTime + ".  This will cause stuttering movement when arm swinging.  Consider changing your Fixed Timestep to " + 1f / 90f + " (90 steps per second) by going to Edit -> Project Settings -> Time -> Fixed Timestep.");
			}
		}
    }
	...
	public void triggerRewind(PreventionReason reason = PreventionReason.MANUAL) {
        ...
        //Debug.Log(Time.frameCount + "|DungeonArmSwinger.triggerRewind():: Rewind triggered due to " + reason);
	...
	static Vector3 calculateCameraRigRewindPosition(Vector3 cameraRigPreviousPosition, Vector3 headsetPreviousPosition, Vector3 cameraRigPosition, Vector3 headsetPosition, PreventionMode preventionMode) {
		...
		Vector3 headsetPositionDifference = DungeonArmSwinger.vector3XZOnly(headsetPreviousPosition) - DungeonArmSwinger.vector3XZOnly(headsetPosition);
	...
	float smoothedControllerMovement(LinkedList<float> controllerMovementHistory) {
		...
		// Functions the same as "Lowest" if the cache size is 2
			...
			Debug.LogError("DungeonArmSwinger.smoothedControllerMovement():: Invalid value for controllerSmoothingMode!");
			return 0;
		}			
	}
	...
	// Allows other scripts and DungeonArmSwinger mechanisms to artifically move the player without a rewind happening
	...
	public float armSwingBothControllersCoefficient {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:armSwingBothControllersCoefficient:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	...
	public float armSwingSingleControllerCoefficient {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:armSwingSingleControllerCoefficient:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	}
	...
	public int raycastAverageHeightCacheSize {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:raycastAverageHeightCacheSize:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	}
	...
	public float preventWallClipMinAngleToTrigger {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:preventWallClipMinAngleToTrigger:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	}
	...
	public float preventClimbingMaxAnglePlayerCanClimb {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:preventClimbingMaxAnglePlayerCanClimb:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	}
	...
	public float preventFallingMaxAnglePlayerCanFall {
		...
		set {
			...
			if (value >= min && value <= max) {
				...
				Debug.LogWarning("DungeonArmSwinger:preventFallingMaxAnglePlayerCanFall:: Requested new value " + value + " is out of range (" + min + ".." + max + ")");
			}
		}
	}
